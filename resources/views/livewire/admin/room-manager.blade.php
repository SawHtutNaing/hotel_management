<div>
                ...
                @foreach($rooms as $room)
                    <tr>
                        <td class="px-6 py-4 whitespace-nowrap">
                            @if($room->image)
                                <img src="{{ Storage::url($room->image) }}" alt="{{ $room->room_number }}" class="h-16 w-16 object-cover rounded">
                            @else
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <button wire:click="editRoom({{ $room->id }})" class="text-blue-600 hover:text-blue-900 mr-4">Edit</button>
                            <button wire:click="deleteRoom({{ $room->id }})" class="text-red-600 hover:text-red-900"
    ...
    <div wire:model="showCreateModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full @if(!$showCreateModal) hidden @endif">
        <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <h2 class="text-lg font-bold mb-4">Create New Room</h2>
            <form wire:submit.prevent="createRoom">
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700">Room Type</label>
                    <select wire:model="room_type_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" wire:model="room_number" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                ...
                <div class="mb-4">
                    ...
                    <input type="checkbox" wire:model="is_available" class="mt-1 rounded-md">
